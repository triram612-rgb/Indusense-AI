...
import { AlertTriangle, Loader2, UploadCloud, XCircle } from 'lucide-react';
      ...
      <div className="grid gap-5 p-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        ...
        <div>
          ...
          <button
            ...
            className={cn(
              'flex min-h-[220px] w-full flex-col items-center justify-center gap-2 rounded-panel border border-dashed border-edge bg-surface/50 p-4 text-center transition-colors hover:border-signal',
              previewUrl && 'p-0',
            )}
          >
            {previewUrl ? (
              <img src={previewUrl} alt="Uploaded unit" className="max-h-[220px] w-full rounded-panel object-contain" />
            ) : (
              <>
                ...
                <span className="text-2xs text-ink-faint">Real inference from the trained classifier — try a defect photo</span>
              </>
            )}
          </button>
        ...
        <div className="flex flex-col justify-center">
          ...
          {result && !loading && !error && (
            <div>
              <div className="flex items-baseline gap-2">
                <span
                  className="readout text-lg font-medium capitalize"
                  style={{ color: CLASS_COLORS[result.predicted_class] ?? '#E7ECF3' }}
                >
                  {result.predicted_class}
                </span>
                <span className="text-xs text-ink-faint">{(result.confidence * 100).toFixed(1)}% confidence</span>
              </div>
              ...
              <p className="mt-3 border-t border-hairline/60 pt-2.5 text-2xs leading-relaxed text-ink-faint">
                Real output from the classifier trained on the provided dataset — not a stored example.
              </p>
            </div>
          )}
          ...
          {!loading && !error && !result && (
            <p className="text-2xs leading-relaxed text-ink-faint">
              Upload a surface photo to see the trained model classify it live, with its actual
              per-class probabilities — same model backing the numbers elsewhere on this page.
            </p>
          )}
        </div>
      </div>
